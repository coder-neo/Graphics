...
scene.loadObjectFiles = function loadObjectFiles(objModels)
{
	...
	// for (i = 0; i < objModels.length; i++) {
		object = new Object();
		object.groups = new Array();
		object.vertices = new Array();
		object.vertexNormal = new Array();
		object.vertexTexture = new Array();
		groups = [];
		vertices = [];
		vertexTexture = [];
		vertexNormal = [];
		groupIndex = -1;
		scene.loadFile("models/"+objModels,"objFile");
    ...
    for ( j = 0; j < groups.length; j++) {
		 if(groups[j].mtlName == null)
		 {
		 	groups[j].mtlName = "Default";
		 	groups[j].Ka.push.apply(groups[j].Ka,unitVector);
		 	groups[j].Kd.push.apply(groups[j].Kd,unitVector);
		 	groups[j].Ks.push.apply(groups[j].Ks,unitVector);
		 }

		if(groups[j].Ka.length == 0)
		{
			groups[j].Ka.push.apply(groups[j].Ka,zeroVector);
		}
		
		if(groups[j].Kd.length == 0)
		{
			groups[j].Kd.push.apply(groups[j].Kd,zeroVector);
		}
		
		if(groups[j].Ks.length == 0)
		{
			groups[j].Ks.push.apply(groups[j].Ks,zeroVector);
		}
	 }
	...
	object.vertexNormal = vertexNormal;
	object.vertexTexture = vertexTexture;
	object.minX = minVal[0];		object.maxX = maxVal[0];
	object.minY = minVal[1];		object.maxY = maxVal[1];
	object.minZ = minVal[2];		object.maxZ = maxVal[2];

	object.center = new Array();
	object.center = objCenter;
